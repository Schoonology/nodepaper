...
function start() {
  ...
  return self._load()
    .then(function () {
      ...
      fs.watchFile(path.join(self.themedir, THEME_INDEX), function (curr, prev) {
        self._load()
      })
...
// Internal use only.
//
// Returns a promise to be fulfilled when the theme has been loaded.
//
Theme.prototype._load = _load
function _load() {
  var self = this

  debug('Loading theme from %s...', path.join(self.themedir, THEME_INDEX))
